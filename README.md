...
- **Κάθε ομάδα κλείνει με ακριβώς 5 παίκτες** και κανένας από τους 10 δεν μένει αδιάθετος. Αυτό το εγγυώνται τρεις κανόνες:
  - **Υποχρεωτική πρώτη προσφορά.** Όποιος ανοίγει τον γύρο δεν μπορεί να αποχωρήσει — πρέπει να δηλώσει τουλάχιστον 1.
  - **Αποθεματικό ανά κενή θέση.** Μετά από κάθε αγορά πρέπει να σου μένει τουλάχιστον 1 για κάθε παίκτη που σου λείπει
    ακόμα, δηλαδή `μέγιστη προσφορά = budget − κενές θέσεις + 1`. Με budget 20 και άδειο ρόστερ δίνεις το πολύ 16.
  - **Αυτόματη ανάθεση.** Μόλις ο ένας συμπληρώσει την πεντάδα του, οι υπόλοιποι παίκτες πάνε αυτόματα στον άλλον για 1,
    χωρίς δημοπρασία.
- Καμία προσφορά πάνω από το διαθέσιμο budget.
- **Δεν υπάρχει περιορισμός θέσης στο ρόστερ.** Μπορείς να καταλήξεις με δύο σέντερ και κανέναν πλέι μέικερ — είναι στρατηγική επιλογή.
  Ο περιορισμός είναι μόνο στο πλήθος (5), όχι στις θέσεις.
- Στο τέλος συγκρίνονται τα συνολικά ratings των δύο πεντάδων.
