...
import { applyBid, minimumBid } from './engine.ts';
...
Deno.serve(async (req: Request) => {
  ...
  // Χωρίς ποσό, θεωρείται προσφορά ακριβώς ένα βήμα πάνω από την τρέχουσα.
  const amount = Math.trunc(body.amount ?? minimumBid(loaded.state));
  const result = applyBid(loaded.state, seat, amount);
  ...
  await recordBidEvent(client, body.gameId, result.state.round, seat, 'bid', amount);

  return json({ highestBid: result.state.highestBid, turnSeat: result.state.turnSeat });
});
